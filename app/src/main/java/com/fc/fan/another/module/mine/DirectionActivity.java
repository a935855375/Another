...
import com.fc.fan.another.adpater.DirectItemAdapter;
...
public class DirectionActivity extends RxBaseActivity {
    public static final String TAG = DirectionActivity.class.getSimpleName();

    @BindView(R.id.direction_toolbar)
    Toolbar toolbar;

    @BindView(R.id.direction_recyclerView)
    RecyclerView recyclerView;

    @BindView(R.id.direction_swipe_refresh)
    ...
    private DirectItemAdapter adapter;
    ...
    public void initViews(Bundle savedInstanceState) {
        ...
        uid = bean.getUid();
        list = new ArrayList<>();
        adapter = new DirectItemAdapter(list);
    ...
    private void requestData(int page, int uid, final boolean isClear) {
        ...
        HttpUtils.getInstance()
                ...
                .getMyDirection(page, uid)
                .compose(bindToLifecycle())
                .doOnNext(data -> {
                    if (data.getList().size() < data.getLimit())
                        isEnd = true;
                })
                .map(DirectBean::getList)
